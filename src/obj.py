...
class Waypoint(LatLon):
    ...
    def __str__(self):
        return "Name: %s, Lat: %f, Lon: %f" % (self.name, self.lat, self.lon)
...
class Geocache(Waypoint):
    ...
    def __str__(self):
        return "Name: %s, Lat: %f, Lon: %f, Difficulty: %.2f, Size: %d" % (self.name, self.lat, self.lon, self.difficulty, self.size)
