...
while True:
    ...
    inp = input('Please choose (r)ock, (p)aper, or (s)cissors. Q to quit\n')
    ...
    if inp.lower() == 'q':
        ...
        print(f'Program Choice: {name[program_choice]}\nYour Choice: {name[inp]}')
        print(dict_to_use[program_choice])
        scorer[dict_to_use[program_choice]]  += 1
        print(f"Wins: {scorer['Win']} Losses: {scorer['Lose']} Ties: {scorer['Tie']}")